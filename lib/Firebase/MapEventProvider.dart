...
class EventTableFromDB extends ChangeNotifier {
  ...
  EventTableFromDB(this.firebaseDB) {
    fireBaseDBSubScriptionInProvider = firebaseDB.child('event').onValue.listen((Event event) {
      _getEventData();
    });
  }
  ...
  Future<void> _getEventData() async {
    //list clear
    testinglist.clear();
    debugPrint('ProviderTest: right after clear and the length is ${testinglist.length}');
    eventMap.clear();
    debugPrint('ProviderTest: right after clear and the eventlist length is ${eventMap.length}');
    ...
    for (var element in eventID_map.keys.toList()) {
      //Obtain prop table
      final innersnapshot = await firebaseDB.child('event').child(element).once();
      ...
      for (var key in property_map.keys.toList()) {
        if (key == 'lattitude') {
          ...
          if (property_map[key] != null) {
            final natureSnapshot = await firebaseDB.child('event').child(element).child('eventNature').once();
            eventNature = natureSnapshot.value;
          }
        } else if (key == 'eventForm') {
          if (property_map[key] != null) {
            final formSnapshot = await firebaseDB.child('event').child(element).child('eventForm').once();
      ...
      eventMap[eventName] = MapEvent(LatLng(lattitude, longitude), eventName, eventHost, eventAddress, eventDescription, startDate, endDate, eventNature, eventForm);
      testinglist.add(innersnapshot.value);
    }

    debugPrint('ProviderTest: end entering and the length is ${testinglist.length}');
    debugPrint('ProviderTest: end entering and the length of event Map is ${eventMap.length}');
